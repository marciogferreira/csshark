...
class MaisVendidosServices extends BaseServices {
    ...
    public function index($request) {
        ...
        if(isset($params['vendedor_id']) && !empty($params['vendedor_id'])) {
            $where .= " AND p.vendedor_id = ".$params['vendedor_id'];
        }
        ...
        $data = DB::table(DB::Raw("
            (
                SELECT sum(ip.quantidade) as quantidade, ip.produto_id, pro.titulo
                FROM pedidos as p
        ...
        "))
        ->select('t.*')
        ->orderBy(DB::raw("titulo"), $params['ordem'])
        ->paginate(10);

        foreach($data as $item) {
            $item->id = $item->produto_id;
            $item->produto = Produto::find($item->produto_id);
            $item->total = $item->quantidade * $item->produto->valor;
